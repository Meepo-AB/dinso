...
const selectedPlan = computed(
  () => props.plans.find((plan) => plan.id === draft.value.planId)?.name ?? '',
)
const canContinue = computed(() =>
  step.value === 1
    ? draft.value.name.trim().length > 1 &&
      draft.value.salary > 0 &&
      Boolean(draft.value.startsOn)
...
<template>
  <section class="employee-flow" aria-labelledby="add-employee-title">
    <div class="employee-flow__topbar">
      <button class="back-link" type="button" @click="emit('cancel')">
        {{ t('Tillbaka till medarbetare') }}
      </button>
    ...
    <header class="employee-flow__header">
      ...
      <p>
        {{
          t(
            'Registrera anställningen i tre korta steg. Ändringen sparas bara här.',
          )
        }}
      </p>
    </header>
    ...
    <ol class="stepper" :aria-label="t('Steg i registreringen')">
      <li :class="{ 'is-current': step === 1, 'is-complete': step > 1 }">
        <span>1</span>
        {{ t('Uppgifter') }}
      </li>
      <li :class="{ 'is-current': step === 2, 'is-complete': step > 2 }">
        <span>2</span>
        {{ t('Avtal') }}
      </li>
    ...
    <form
      class="employee-flow__body"
      @submit.prevent="step === 3 ? submit() : next()"
    >
      <fieldset v-if="step === 1" class="form-section">
        <legend>{{ t('Medarbetarens uppgifter') }}</legend>
        <p>
          {{
            t('Ange uppgifterna som behövs för att starta en ny anställning.')
          }}
        </p>
        <div class="field-grid">
          <label class="field field--wide">
            ...
            <input
              ...
              required
              autocomplete="name"
              :placeholder="t('Till exempel Kim Andersson')"
            />
          </label>
          <label class="field">
      ...
      <fieldset v-else-if="step === 2" class="form-section">
        ...
        <p>
          {{
            t('Medarbetaren ansluts till vald plan från och med startdatumet.')
          }}
        </p>
        <div class="plan-options">
          <label
            ...
            :key="plan.id"
            class="plan-option"
            :class="{ 'is-selected': draft.planId === plan.id }"
          >
            <input
            ...
            <span>
              ...
              <small>{{ t('Aktiv pensionsplan') }}</small>
            </span>
          </label>
        </div>
      </fieldset>
      ...
      >
        ...
        <p>
          {{
            t(
              'Kontrollera uppgifterna. Du kan gå tillbaka och ändra dem innan registrering.',
            )
          }}
        </p>
        <dl>
          ...
          <div>
            <dt>{{ t('Månadslön') }}</dt>
            <dd>{{ draft.salary.toLocaleString('sv-SE') }} {{ t('kr') }}</dd>
          </div>
          <div>
            <dt>{{ t('Pensionsplan') }}</dt>
            <dd>{{ selectedPlan }}</dd>
          </div>
        </dl>
      </section>
...
.employee-flow__header {
  max-width: 620px;
  margin-bottom: 32px;
}
.employee-flow__header h1 {
  margin: 0;
  font-family: var(--font-display, var(--font-sans));
  font-size: clamp(1.75rem, 3.2vw, 2.5rem);
  letter-spacing: var(--heading-tracking, -0.02em);
}
.employee-flow__header p,
.form-section > p {
  color: var(--muted);
  line-height: 1.55;
}
