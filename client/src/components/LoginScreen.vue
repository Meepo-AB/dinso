...
const props = defineProps<{
  ...
  portals: { id: Portal; title: string; description: string; available: boolean }[]
...
const selectedProfile = () => props.profiles.find(item => item.id === props.selectedProfileId) ?? null
</script>
    ...
    <div class="login-screen__frame">
      <header class="login-screen__intro">
        <p class="login-screen__brand">
          <img class="login-screen__icon" :src="customerIconUrl" alt="" aria-hidden="true" />
          {{ customerName }}
        </p>
      ...
      <section class="login-screen__profiles" aria-labelledby="profile-heading">
        ...
        <div class="profile-cards" role="radiogroup" :aria-label="labels.profileLabel">
          <button
          ...
          >
            ...
            <span class="profile-card__description">{{ item.description }}</span>
      ...
      <section class="login-screen__portals" aria-labelledby="portal-heading">
        ...
        <div v-else class="portal-cards" role="radiogroup" :aria-label="labels.portalLabel">
          <button
            v-for="portal in portals.filter(item => item.available)"
            :key="portal.id"
            type="button"
            class="portal-card"
          ...
          >
            ...
            <span class="portal-card__body">
              ...
              <span class="portal-card__description">{{ portal.description }}</span>
            </span>
          </button>
        </div>

        <p v-if="loginError" class="login-screen__error" role="alert">{{ loginError }}</p>

        <div class="login-screen__submit">
          <p class="login-screen__hint">{{ selectedPortal ? labels.switchHint : labels.hint }}</p>
          <button class="button" type="button" :disabled="!selectedProfile() || !selectedPortal" @click="emit('submit')">
            {{ labels.open }}
          </button>
        </div>
      </section>
