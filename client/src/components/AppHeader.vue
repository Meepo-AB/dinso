  ...
  <header class="app-header">
    <div class="app-header__identity">
      <span class="app-header__brand">
        <img class="app-header__icon" :src="customerIconUrl" alt="" aria-hidden="true" />
        {{ customerName }}
    ...
    <div class="app-header__profile">
      ...
      <label v-if="companies && companies.length > 1" class="app-header__company">
        <span>{{ companyLabel }}</span>
        <select :value="selectedCompanyId" @change="$emit('select-company', ($event.target as HTMLSelectElement).value)">
          <option v-for="company in companies" :key="company.id" :value="company.id">{{ company.name }}</option>
        </select>
      </label>
      <button class="app-header__signout" type="button" @click="$emit('sign-out')">
        {{ signOutLabel }}
      </button>
    </div>
  </header>
...
.app-header {
  ...
  padding: var(--space-header-padding, 18px) max(24px, calc((100vw - var(--shell-max-width, 1440px)) / 2));
...
.app-header__company select {
  min-height: 36px;
  max-width: 220px;
  border: 1px solid color-mix(in srgb, var(--header-ink, white) 55%, transparent);
  border-radius: var(--radius-control);
  background: var(--header-bg, var(--ink));
  color: var(--header-ink, white);
  font: inherit;
