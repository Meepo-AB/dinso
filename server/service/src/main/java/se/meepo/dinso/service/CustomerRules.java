...
public record CustomerRules(
  ...
  public CustomerRules {
    ...
    if (portals.isEmpty() || locales.isEmpty() || !locales.contains(defaultLocale))
      throw new IllegalArgumentException(
          "Customer rules require supported portals and a valid default locale");
    if (maximumFunds < 1 || maximumLeaveMonths < 0)
      throw new IllegalArgumentException("Customer limits must not be negative");
    if (!portals.contains(PortalType.COMPANY)
        && (!companyMutations.isEmpty() || maximumLeaveMonths != 0 || !leaveReasons.isEmpty()))
      throw new IllegalArgumentException("Private-only customers cannot have company rules");
  }
}
