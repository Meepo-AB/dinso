...
public class DemoSessionService {
  ...
  public DemoProfile requireActive(String token) {
    ...
    var session = sessions.findByToken(claims.sessionId()).filter(item -> item.isActive() && item.getExpiresAt().isAfter(clock.instant()) && item.getProfile().getCustomerId() == claims.customer()).orElseThrow(() -> new SecurityException("Invalid or expired demo session"));
    return session.getProfile().toDomain();
  }
  public void logout(String token) { sessions.findByToken(jwt.verify(token).sessionId()).ifPresent(session -> session.invalidate()); }
}
