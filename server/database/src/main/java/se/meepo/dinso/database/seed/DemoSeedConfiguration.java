...
public class DemoSeedConfiguration {
  ...
  CommandLineRunner seedDemoData(
      CustomerId customer, DemoProfileRepository profiles, DemoDataSeeder dataSeeder) {
    return ignored -> {
      DemoCatalog.profiles(customer)
          .forEach(
              profile ->
                  profiles
                      .findByCustomerIdAndExternalId(customer, profile.id())
                      .orElseGet(() -> profiles.save(new DemoProfileEntity(profile))));
